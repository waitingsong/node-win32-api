...
// ---------------------------------------------
async function cpGlobalConfigsToPkgs(baseDir, configPaths, pkgBase) {
    ...
    for (const pkg of pkgs) {
        for (const path of configPaths) {
            try {
                ...
                await cp(join(baseDir, path), join(pkgBase, dst));
                ret.push(dst);
            }
            catch (ex) {
...
//# sourceMappingURL=gen-file-from-example.js.map
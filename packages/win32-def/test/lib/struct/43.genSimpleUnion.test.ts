...
import { genSimpleUnion } from '##/index.struct.js'


describe(fileShortPath(import.meta.url), () => {
  describe('genSimpleUnion()', () => {
    const expectSize = 4
    const u2 = {
    ...
    it('normal', async () => {
      ...
      const { name, pointer, CType, size } = genSimpleUnion(u2, key, ptr)
    ...
    it('pass no ptr', async () => {
      const key = 'uu2'
      const { name, pointer, CType, size } = genSimpleUnion(u2, key)
